...
def vectorDistance2(v1, v2):
    """
    this function calculates de euclidean distance between two 
    vectors.
    """
    weight_s = 0.00293727808493**2 + 0.001**2 
    sum = 0
    sum = ((0.00293727808493**2/weight_s))*((v1[0] - v2[0]) ** 2) + ((0.001**2/weight_s))*((v1[1] - v2[1]) ** 2)
    return sum ** 0.5
#  INTERNOS
...
class geoSomManager(somManager):
    ...
    def defGeoWinnerAttributes(self):
        ...
        for c in self.iCentroids:
            bestOIndex = 0
            minDistance = vectorDistance2(self.iCentroids[c], self.oCentroids[0])
            outputContiguity = self.outputContiguity[0]
            k = 4*0.003
            for o in self.oCentroids:
                dis = vectorDistance2(self.iCentroids[c], self.oCentroids[o])
                if dis < k:
            ...
            #print "feasible BMU: " , feasibleBMU[c]
